...
DB_PATH = str(Path(**file**).parent / “booking.db”)
...
con.execute(“PRAGMA journal_mode=WAL”)     # параллельное чтение во время записи
con.execute(“PRAGMA synchronous=NORMAL”)   # быстрее без потери надёжности
con.execute(“PRAGMA cache_size=-10000”)    # 10 MB кэш в памяти
con.execute(“PRAGMA temp_store=MEMORY”)    # временные таблицы в RAM
return con
...
```
    ...
    # Миграция: заполнить clients из существующих записей
    c.execute("SELECT COUNT(*) FROM clients")
    if c.fetchone()[0] == 0:
        c.execute("""
            INSERT OR IGNORE INTO clients (user_id, username, name, phone, first_seen, last_seen)
...
upsert_client(user_id, username, full_name, phone)
return appt_id
...
def upsert_client(user_id, username, name, phone):
now = datetime.now().strftime(”%Y-%m-%d %H:%M:%S”)
with db() as con:
con.execute(”””
INSERT INTO clients (user_id, username, name, phone, first_seen, last_seen)
VALUES (?,?,?,?,?,?)
...
“””, (user_id, username or ‘’, name or ‘’, phone or ‘’, now, now))

def get_clients_stats():
with db() as con:
return con.execute(”””
SELECT
c.user_id, c.name, c.phone, c.username,
...
WHERE a2.user_id = c.user_id AND a2.status = ‘active’
GROUP BY s2.name ORDER BY COUNT(*) DESC LIMIT 1) AS fav_service
FROM clients c
LEFT JOIN appointments a ON a.user_id = c.user_id AND a.status = ‘active’
LEFT JOIN services s ON a.service_id = s.id
GROUP BY c.user_id
ORDER BY visits DESC, c.last_seen DESC
“””).fetchall()
...
“SELECT COALESCE(SUM(s.price),0) FROM appointments a JOIN services s ON a.service_id=s.id WHERE a.status=‘active’”
).fetchone()[0]
total_appts     = con.execute(“SELECT COUNT(*) FROM appointments WHERE status=‘active’”).fetchone()[0]
this_month      = datetime.now().strftime(”%Y-%m”)
month_revenue   = con.execute(
“SELECT COALESCE(SUM(s.price),0) FROM appointments a JOIN services s ON a.service_id=s.id WHERE a.status=‘active’ AND a.date LIKE ?”,
(f”{this_month}%”,)).fetchone()[0]
month_appts     = con.execute(
“SELECT COUNT(*) FROM appointments WHERE status=‘active’ AND date LIKE ?”,
(f”{this_month}%”,)).fetchone()[0]
top_service     = con.execute(”””
SELECT s.name, COUNT(*) as cnt
FROM appointments a JOIN services s ON a.service_id=s.id
WHERE a.status=‘active’
GROUP BY s.name ORDER BY cnt DESC LIMIT 1
“””).fetchone()
regulars        = con.execute(”””
SELECT COUNT(*) FROM (
SELECT user_id FROM appointments WHERE status=‘active’
...
def get_broadcast_targets(filter_type=‘all’):
with db() as con:
if filter_type == ‘all’:
rows = con.execute(“SELECT user_id FROM clients”).fetchall()
elif filter_type == ‘active’:
cutoff = (datetime.now() - timedelta(days=90)).strftime(”%Y-%m-%d”)
rows = con.execute(
“SELECT DISTINCT user_id FROM appointments WHERE status=‘active’ AND date>=?”,
(cutoff,)).fetchall()
elif filter_type == ‘new’:
cutoff = (datetime.now() - timedelta(days=30)).strftime(”%Y-%m-%d”)
rows = con.execute(
“SELECT user_id FROM clients WHERE first_seen>=?”,
(cutoff,)).fetchall()
else:
rows = []
return [r[0] for r in rows]
...
output  = io.StringIO()
output.write(’\ufeff’)  # BOM — Excel читает кириллицу без проблем
w = csv.writer(output, delimiter=’;’)
w.writerow([‘Имя’, ‘Телефон’, ‘Telegram’, ‘Визитов’, ‘Сумма ₽’,
‘Любимая услуга’, ‘Первый визит’, ‘Последний визит’])
for uid, name, phone, username, first_seen, last_seen, visits, total_spent, fav in clients:
w.writerow([
name or ‘—’,
phone or ‘—’,
...
await ctx.bot.send_message(
    ...
    f"─────────────────────\n"
    f"👤  {esc(cname)}  {esc(uname)}\n"
    f"📞  {esc(phone)}\n"
    f"💅  {esc(svc[1])}\n"
    f"💰  {fmt_price(svc[2])}\n"
    f"📅  {fmt_date(date)}  🕐  {time}\n"
    + (f"💬  {esc(cmt)}" if cmt else ""),
...
for appt_id, sname, price, date, time, cmt, status in rows:
    ...
    text += f"{'⏳' if pending else '✅'}  *{esc(sname)}*  —  {fmt_price(price)}\n"
...
f”{‘Нажмите кнопку ниже 👇’ if uname else ‘*Username мастера пока не указан.*’}”,
reply_markup=InlineKeyboardMarkup(kb),
parse_mode=ParseMode.MARKDOWN)
...
[InlineKeyboardButton(“👥  База клиентов”,   callback_data=“a:clients”)],
...
open_ = sum(1 for *, o in slots if o)
...
if slots: label += f”  •  {open*}/{len(slots)}”
...
f”*Формат времени: ЧЧ:ММ, например* `09:30`\n”
f”*Шаг: 30, 45, 60, 90 или 120 минут*\n\n”
...
for appt_id, fname, uname, sname, date, time, cmt, uid, phone, status in rows:
    ...
    text += f"👤  {esc(fname)}  {esc(u)}\n"
    if phone: text += f"📞  {esc(phone)}\n"
    text += f"💅  {esc(sname)}\n"
    if cmt: text += f"💬  {esc(cmt)}\n"
    text += "\n"
    row = []
...
@guard
async def a_clients(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
q = update.callback_query; await q.answer()
clients = get_clients_stats()
...
if not clients:
    return await q.edit_message_text(
        "👥 *База клиентов*\n\nКлиентов пока нет — они появятся после первых записей.",
        reply_markup=InlineKeyboardMarkup(back("admin")),
        parse_mode=ParseMode.MARKDOWN)
...
text = (
    f"👥 *База клиентов*\n"
    f"─────────────────────\n"
    f"Всего: *{stats['total_clients']}* чел.\n"
    f"Выручка за месяц: *{fmt_price(stats['month_revenue'])}*\n"
    f"Всего заработано: *{fmt_price(stats['total_revenue'])}*\n"
    f"Популярная услуга: *{esc(stats['top_service'])}*\n"
    f"─────────────────────\n"
    f"_Нажмите на клиента для подробностей:_"
)

kb = []
for uid, name, phone, username, first_seen, last_seen, visits, total_spent, fav in clients[:20]:
    label = f"{esc(name or phone or str(uid))}  •  {visits} визит{'а' if 2<=visits<=4 else 'ов' if visits>=5 else ''}"
    kb.append([InlineKeyboardButton(label, callback_data=f"a:client:{uid}")])

if len(clients) > 20:
    kb.append([InlineKeyboardButton(f"…ещё {len(clients)-20} клиентов в экспорте", callback_data="a:export")])

kb.append([InlineKeyboardButton("📥  Скачать всех в CSV", callback_data="a:export")])
kb += back("admin")
...
@guard
async def a_client_card(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
q   = update.callback_query; await q.answer()
...
with db() as con:
    row = con.execute("""
        SELECT c.user_id, c.name, c.phone, c.username,
               c.first_seen, c.last_seen,
               COUNT(a.id)              AS visits,
               COALESCE(SUM(s.price),0) AS total_spent,
...
uid, name, phone, username, first_seen, last_seen, visits, total_spent, fav = row
...
text = (
    ...
    f"📊 *Статистика*\n"
    f"Визитов: *{visits}*\n"
    f"Потрачено: *{fmt_price(total_spent)}*\n"
    f"Любимая услуга: *{esc(fav or '—')}*\n"
    f"Первый визит: {(first_seen or '')[:10]}\n"
    f"Последний: {(last_seen or '')[:10]}\n"
)

kb = []
if username:
    kb.append([InlineKeyboardButton("💬  Написать клиенту", url=f"https://t.me/{username}")])
kb += back("a:clients")
...
@guard
async def a_export(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
q = update.callback_query; await q.answer(“Генерирую файл…”)

```
clients = get_clients_stats()
if not clients:
    return await q.answer("База клиентов пуста.", show_alert=True)
...
await ctx.bot.send_document(
    ...
    filename=filename,
    caption=(
        f"📥 *База клиентов* — {len(clients)} чел.\n"
        f"Открывается в Excel и Google Sheets.\n"
        f"Для рекламных кабинетов используйте колонку «Телефон»."
    ),
    parse_mode=ParseMode.MARKDOWN)
```
...
@guard
async def a_broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
q = update.callback_query; await q.answer()

```
all_count    = len(get_broadcast_targets('all'))
active_count = len(get_broadcast_targets('active'))
new_count    = len(get_broadcast_targets('new'))

kb = [
    [InlineKeyboardButton(f"📣  Всем клиентам ({all_count})",          callback_data="a:bc_filter:all")],
    [InlineKeyboardButton(f"🔁  Активным за 90 дней ({active_count})", callback_data="a:bc_filter:active")],
    [InlineKeyboardButton(f"🆕  Новым за 30 дней ({new_count})",       callback_data="a:bc_filter:new")],
    *back("admin"),
]
await q.edit_message_text(
    "📢 *Рассылка*\n─────────────────────\n"
    "Выберите аудиторию:\n\n"
    "_⚠️ Не злоупотребляйте рассылками — клиенты могут заблокировать бот._",
    reply_markup=InlineKeyboardMarkup(kb),
    parse_mode=ParseMode.MARKDOWN)
```

@guard
async def a_bc_filter(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
q = update.callback_query; await q.answer()
filter_type = q.data.split(”:”)[2]
targets     = get_broadcast_targets(filter_type)

```
labels = {'all': 'все клиенты', 'active': 'активные за 90 дней', 'new': 'новые за 30 дней'}

ctx.user_data['broadcast_filter']  = filter_type
ctx.user_data['broadcast_targets'] = targets
ctx.user_data['state']             = 'broadcast_text'

await q.edit_message_text(
    f"📢 *Рассылка • {labels[filter_type]}*\n"
    f"─────────────────────\n"
    f"Получат: *{len(targets)}* чел.\n\n"
    f"Напишите текст сообщения.\n"
    f"_Поддерживается *жирный*, _курсив_, ссылки._",
    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("← Отмена", callback_data="a:broadcast")]]),
    parse_mode=ParseMode.MARKDOWN)
```
...
if state == "comment":
...
elif state == "broadcast_text" and is_admin(update.effective_user.id):
    targets = ctx.user_data.get('broadcast_targets', [])
    ...
    kb = [
        [InlineKeyboardButton("✅  Отправить", callback_data="a:bc_send")],
        [InlineKeyboardButton("✏️  Изменить",  callback_data=f"a:bc_filter:{ctx.user_data.get('broadcast_filter','all')}")],
        *back("a:broadcast"),
    ]
...
@guard
async def a_bc_send(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
q = update.callback_query; await q.answer(“Отправляю…”)
...
if not targets or not message:
    return await q.answer("Нет данных для рассылки.", show_alert=True)

await q.edit_message_text(
    f"⏳ *Рассылка запущена*\nОтправляю {len(targets)} клиентам…",
    parse_mode=ParseMode.MARKDOWN)
...
ctx.user_data.pop('broadcast_targets', None)
ctx.user_data.pop('broadcast_message', None)
ctx.user_data.pop('broadcast_filter',  None)

await ctx.bot.send_message(
    ADMIN_ID,
    f"✅ *Рассылка завершена*\n\n"
    f"Доставлено: *{sent}*\n"
    f"Не доставлено: *{failed}* _(заблокировали бот)_",
    reply_markup=InlineKeyboardMarkup(back("admin")),
    parse_mode=ParseMode.MARKDOWN)
```
...
“”“Логирует ошибку и отправляет мастеру уведомление.”””
logging.error(“Uncaught exception:”, exc_info=ctx.error)
msg = f”⚠️ *Ошибка в боте*\n\n`{str(ctx.error)[:400]}`”
try:
await ctx.bot.send_message(ADMIN_ID, msg, parse_mode=ParseMode.MARKDOWN)
except Exception:
pass  # если не смогли отправить — просто логируем

async def job_backup(ctx: ContextTypes.DEFAULT_TYPE):
“”“Ежедневная отправка базы данных мастеру в Telegram.”””
try:
# Делаем горячий бэкап через SQLite API (корректно даже под нагрузкой)
backup_path = str(Path(DB_PATH).parent / “backup_temp.db”)
...
```
    with open(backup_path, "rb") as f:
        await ctx.bot.send_document(
            ADMIN_ID,
            document=f,
            filename=f"booking_backup_{datetime.now().strftime('%Y%m%d')}.db",
            caption=(
                f"🗄 *Автобэкап базы данных*\n"
                f"📅 {datetime.now().strftime('%d.%m.%Y %H:%M')}\n\n"
                f"_Сохраните этот файл на всякий случай._"
            ),
            parse_mode=ParseMode.MARKDOWN)
...
app = (
    ...
    .token(BOT_TOKEN)
    .post_init(post_init)
    .concurrent_updates(True)   # одновременная обработка нескольких пользователей
    .read_timeout(30)           # ждать ответа от Telegram до 30 сек
    .write_timeout(30)          # ждать подтверждения отправки до 30 сек
    .connect_timeout(15)        # таймаут установки соединения
...
app.add_handler(CallbackQueryHandler(a_clients,       pattern="^a:clients$"))
app.add_handler(CallbackQueryHandler(a_client_card,   pattern=r"^a:client:\d+$"))
app.add_handler(CallbackQueryHandler(a_export,        pattern="^a:export$"))
app.add_handler(CallbackQueryHandler(a_broadcast,     pattern="^a:broadcast$"))
app.add_handler(CallbackQueryHandler(a_bc_filter,     pattern=r"^a:bc_filter:"))
app.add_handler(CallbackQueryHandler(a_bc_send,       pattern="^a:bc_send$"))
